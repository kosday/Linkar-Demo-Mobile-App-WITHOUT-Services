    ...
    public class CustomerDataStore
    {
        ...
        public async Task<IEnumerable<Customer>> GetCustomersAsync(bool forceRefresh = false)
        {
            if (forceRefresh)
            {
                ...
                try
                {
                    ...
                    string lkstring = App.linkarClt.Select("LK.CUSTOMERS", "", "BY ID", "", "", new SelectOptions(false, false, 10, 1, true), "", 0); //NEWFRAMEWORK: Replace Select_Text for Select, remove DATAFORMATCRU_TYPE.MV
        ...
        public async Task<Customer> GetCustomerAsync(string id)
        {
            ...
            {                
                try
                {
                    string lkstring = App.linkarClt.Read("LK.CUSTOMERS", id, "", new ReadOptions(true), "", 0);  //NEWFRAMEWORK: Replace Read_Text for Read, remove DATAFORMATCRU_TYPE.MV
